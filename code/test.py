...
plt.subplot(332), plt.imshow(abssobelx, cmap = 'gray')
plt.title('Sobel Gradient x'), plt.xticks([]), plt.yticks([])
plt.subplot(333), plt.imshow(abssobely, cmap = 'gray')
...
plt.subplot(334),plt.imshow(gradient,cmap = 'gray')
plt.title('Morphological gradient'), plt.xticks([]), plt.yticks([])


# Morphological gradient threshold
th2 = cv.adaptiveThreshold(gradient,255,cv.ADAPTIVE_THRESH_MEAN_C,\
...
# Canny edge
edges = cv.Canny(gradient,5,8,apertureSize = 3, L2gradient = True)
